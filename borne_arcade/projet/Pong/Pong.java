...
public class Pong {
	...
	public Pong() {
		...
		jouer = new Rectangle(Couleur.BLANC, new Point((largeur - largeurItem) / 2, yItems), largeurItem, hauteurItem,
				true);
		exit = new Rectangle(Couleur.BLANC, new Point((largeur - largeurItem) / 2, yItems - 2 * hauteurItem),
				largeurItem, hauteurItem, true);
		selection = new Rectangle(Couleur.ROUGE,
				new Point((largeur - largeurItem) / 2 - margeSelection, yItems - margeSelection),
				largeurItem + 2 * margeSelection, hauteurItem + 2 * margeSelection, true);
		f.ajouter(selection);
		...
		texteJouer.setA(new Point(((largeur - texteJouer.getLargeur()) / 2),
				yItems + (hauteurItem - texteJouer.getHauteur()) / 2));

		f.ajouter(texteJouer);

		Texte texteExit = new Texte(Couleur.NOIR, "EXIT", new Font("Calibri", Font.TYPE1_FONT, 40), new Point());
		texteExit.setA(new Point(((largeur - texteExit.getLargeur()) / 2),
				(yItems + hauteurItem - texteExit.getHauteur()) / 2));
		f.ajouter(texteExit);

		Texte textPong = new Texte(Couleur.BLANC, "PONG", new Font("Arial", Font.TYPE1_FONT, 250), new Point());
		textPong.setA(
				new Point(((largeur - textPong.getLargeur()) / 2), (((hauteur - textPong.getHauteur()) / 2) + 100)));
		f.ajouter(textPong);

		/*
		 * if(clavier.getJoyJ1HautEnfoncee()) {
		 * if(this.Pointeur == 1) {
		 * selection = new Rectangle(Couleur.BLANC, new
		 * Point((largeur-largeurItem2)/2,yItems-2*hauteurItem2),largeurItem2,
		 * hauteurItem2, true);
		 * f.ajouter(selection);
		 * }
		 * }
	...
	public void GeneratejeuPong() {
		...
		limiteDroite = new Rectangle(Couleur.ROUGE, new Point(largeur - epaisseurLigne, 0), new Point(largeur, hauteur),
				true);
		...
		limiteHaute = new Rectangle(Couleur.BLANC, new Point(0, hauteur - epaisseurLigne), new Point(largeur, hauteur),
				true);
		f.ajouter(limiteHaute);

		Rectangle filet = new Rectangle(Couleur.BLANC, new Point(largeur / 2 - epaisseurLigne / 2, 0),
				new Point(largeur / 2 + epaisseurLigne / 2, hauteur), true);
		f.ajouter(filet);
	...
	public void init() {
		...
		if (scoreG == 9) {
			Texte t = new Texte(Couleur.ROUGE, "Le joueur de gauche a gagné !!!",
					new Font("Calibri", Font.TYPE1_FONT, 40), new Point(largeur / 2, hauteur / 2));
			f.ajouter(t);
			f.rafraichir();
			try {
				Thread.sleep(5000);
		...
		if (scoreD == 9) {
			Texte t = new Texte(Couleur.ROUGE, "Le joueur de droite a gagné !!!",
					new Font("Calibri", Font.TYPE1_FONT, 40), new Point(largeur / 2, hauteur / 2));
			f.ajouter(t);
			f.rafraichir();
			try {
